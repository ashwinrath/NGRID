...
{
    /// <summary>
    /// This class extends NGRIDRemoteApplication, adds functionality to send/receive persistent messages.
    /// </summary>
    public abstract class NGRIDPersistentRemoteApplicationBase : NGRIDRemoteApplication
    {
        #region Consts
        ...
        /// <summary>
        /// Reference to the Store manager in NGRID server.
        /// </summary>
        ...
        protected NGRIDPersistentRemoteApplicationBase(string name) :
        ...
        /// </summary>
        /// <param name="message">Message</param>
        public void AddMessageToHeadOfQueue(NGRIDDataTransferMessage message)
        {
            //Lock collection to be synchronized.
            lock (_waitingMessages)
            {
                ...
                _waitingMessages.AddFirst(
                    new WaitingMessage(
                        new NGRIDMessageRecord(message)
                            {
        ...
        protected override void OnResponseReceived(ICommunicator communicator, NGRIDOperationResultMessage operationResultMessage)
        {
        ...
        {
            Logger.Debug("NGRIDPersistentRemoteApplicationBase - ProcessWaitingMessage thread is started. ApplicationId = " + ApplicationId);
            ...
            Logger.Debug("NGRIDPersistentRemoteApplicationBase - ProcessWaitingMessage thread is stopped. ApplicationId = " + ApplicationId);
        ...
        protected abstract List<NGRIDMessageRecord> GetWaitingMessages(int minId, int maxCount);
        ...
        /// This method is designed, because it is different to get next server's name for client applications and ngrid servers.
        /// </summary>
        /// <returns>Next server</returns>
        protected abstract string GetNextServerForMessage(NGRIDDataTransferMessage message);
        ...
        private class WaitingMessage
        {
            ...
            /// </summary>
            public NGRIDMessageRecord MessageRecord { get; private set; }
