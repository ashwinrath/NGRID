...
{
    /// <summary>
    /// This is a base class for remote application connected to the server as a NGRIDAdjacentServer,
    /// NGRIDClientApplication or NGRIDController.
    /// </summary>
    public abstract partial class NGRIDRemoteApplication : IRunnable
    {
        #region Events
        ...
        /// </summary>
        public NGRIDSettings Settings { set; protected get; }
        ...
        /// <summary>
        /// This queue is used to queue NGRIDMessage objects received from remote application and process them sequentially.
        /// </summary>
        ...
        public void AddCommunicator(ICommunicator communicator)
        {
            lock (_communicators)
            {
                ...
                {
                    throw new NGRIDException("New communicator connection is not allowed, because Communicator count reached to maximum communicator count (" + maxAllowedCommunicatorCount + ")");
                }
        ...
        /// <summary>
        /// Sends a NGRIDMessage to this application.
        /// This method does not block calling thread to wait an ACK for sending message.
        /// This method is just an overload for SendMessage(NGRIDMessage, ICommunicator) method as communicator is null.
        /// </summary>
        ...
        protected virtual void OnResponseReceived(ICommunicator communicator, NGRIDOperationResultMessage operationResultMessage)
        {
            //No action
        }
        ...
        /// <summary>
        /// Processes a NGRIDOperationResultMessage message.
        /// </summary>
        ...
        private void ProcessOperationResultMessage(MessageReceivedFromCommunicatorEventArgs e)
        {
             ...
             * thus, caller of SendDataMessage (NGRIDPersistentRemoteApplicationBase.ProcessWaitingMessage) method 
             * gets response. */
            if (handled)
            {
                OnResponseReceived(e.Communicator, e.Message as NGRIDOperationResultMessage);
            }
        }

        /// <summary>
        /// Processes a NGRIDChangeCommunicationWayMessage message.
        /// </summary>
        /// <param name="e">Event arguments from Communicator_MessageReceived method</param>
        private static void ProcessChangeCommunicationWayMessage(MessageReceivedFromCommunicatorEventArgs e)
        {
            var message = e.Message as NGRIDChangeCommunicationWayMessage;
            if (message == null)
            {
                return;
            }
